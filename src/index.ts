...
async function deliverAlert(env: Env, message: AlertDeliveryMessage): Promise<void> {
  ...
  const row = await env.DB.prepare('SELECT o.id, o.wallet_address, a.simulation_net_pnl, a.max_drawdown_pct, a.adverse_scaling_score FROM alert_outbox o JOIN audits a ON a.id=o.audit_id WHERE o.id=?').bind(message.outboxId).first<{ id: string; wallet_address: string; simulation_net_pnl: number; max_drawdown_pct: number; adverse_scaling_score: number }>();
  if (!row) throw new Error('outbox audit missing');
  const text = `🚨 NEW ELITE POLYMARKET TRADER DETECTED 🚨\n\n• Wallet: <code>${row.wallet_address}</code>\n• Normalized replay P&L: $${row.simulation_net_pnl.toFixed(2)}\n• Simulation Max Drawdown: ${row.max_drawdown_pct.toFixed(1)}%\n• Adverse Scaling Score: ${row.adverse_scaling_score.toFixed(1)}\n\n📈 PROPOSED POLYCOP LIVE GUARDRAILS:\n• Proportional Position Size: 10%\n• Hard Capital Stop Limit: $150 Max Per Market\n• Max Allowed Slippage: 1.5% FAK Cutoff\n\n<a href="https://polycop.ai">👉 Click to Audit Profile via PolyCop Dashboard</a>`;
  try {
